...
if($rows == 0){
	echo "<script>alert('该课已被选择，不可再选！');location.href='general_course.php?p=1';</script>";
}
// print_r($result3);
// print_r($result);
// print_r($result1);
...
if($ok){
	...
	if($r){
		echo "<script>alert('恭喜您，选课成功！');location.href='general_course.php?p=1';</script>";
	}
}else{
	echo "<script>alert('课程上课时间冲突，请重新选择！');location.href='general_course.php?p=1';</script>";
}
?>